...
public class ContactHelper {
     ...
     * @return 联系人列表
     */
    public static List<ContactInfo> getContacts(boolean isClear) {
    ...
    public static void onActivityResult(int reqCode, int resultCode, Intent data, OnGetContactInfo listener) {
        if (resultCode == Activity.RESULT_OK && reqCode == REQUEST_PICK_CONTACT) {
            ...
            ContactInfo info = getContactInfoById(contactData.getLastPathSegment());
            if (listener != null)
     ...
     * 再通过查询到对应的phone
     */
    private static ContactInfo getContactInfoById(String contactId) {
        ...
        Cursor cursor = DevelopLib.getCtx().getContentResolver().query(
    ...
    private static void getPhoneContacts() {
        ContentResolver resolver = DevelopLib.getCtx().getContentResolver();
        //query查询，得到结果的游标
     ...
     */
    public static boolean insert(String name, String phoneNumber) {
        //根据号码找数据，如果存在则不添加
    ...
    public static boolean update(String name, String phoneNumber) {
        try {
            ...
            if (cursor.moveToFirst()) {
                ...
                ContentResolver mResolver = DevelopLib.getCtx().getContentResolver();
                ContentValues values = new ContentValues();
                values.put("data1", phoneNumber);
